<?php
    class Model {
        ...
        * @return [array ou false] Tableau contenant les infos(id, year, name, category, birthdate, birthplace, country, motivation) ou false
        */
        ...
        */
        public function getCategories() {
            try {
                $requete = $this->bd->prepare('select distinct(category) from nobels');
                $requete->execute();
        ...
        * @param [array] $data contient les informations year, name, category, birthdate, birthplace, country, motivation
        ...
        */
        public function addNobelPrize($infos) {
            try {
                ...
                $requete = $this->bd->prepare('INSERT INTO nobels (year, category, name, birthdate, birthplace, country, motivation) VALUES (:year, :category, :name, :birthdate, :birthplace, :country, :motivation)');

                //Remplacement des marqueurs de place par les valeurs
                $marqueurs = ['year', 'category', 'name', 'birthdate','birthplace', 'country', 'motivation'];
        ...
        * @param [array] $data contient les informations id, year, name, category, birthdate, birthplace, country, motivation
        * @return [boolean] retourne true si la personne a été ajoutée dans la base de données, et false sinon
        */
        public function updateNobelPrize($infos) {
            try {
                //Préparation de la requête
                $requete = $this->bd->prepare('UPDATE nobels SET year = :year, category = :category, name = :name, birthdate = :birthdate, birthplace = :birthplace, country = :country, motivation = :motivation WHERE id = :id');

                //Remplacement des marqueurs de place par les valeurs
                $marqueurs = ['id','year', 'category', 'name', 'birthdate','birthplace', 'country', 'motivation'];
                foreach ($marqueurs as $value) {
                    $requete->bindValue(':' . $value, $infos[$value]);
                }
        ...
        */
         ...
         * @return [tableau] retourne un tableau si les conditions sont respectés, et false sinon
         */
        public function check_data(){
            // Vérifie le nom, la catégorie et l'année sinon retourne false
            if ( isset($_POST['Name']) && isset($_POST['Category']) && isset($_POST['Year']) && !preg_match("#^ +$#",$_POST['Name']) && !preg_match("#^ +$#",$_POST['Name']) && preg_match("#^\d{4}$#",$_POST['Year']) ) {
                $infos = ["year"=>$_POST['Year'], "category"=>$_POST['Category'], "name"=>$_POST['Name']];
            }
            else {
                return false;
            }

            // Vérifie la date de naissance sinon valeur=null
            if ( isset($_POST['BirthDate']) && preg_match("#^\d{4}$#",$_POST['BirthDate']) ) {
            ...
            // Vérifie le lieu de naissance sinon valeur=null
            if ( isset($_POST['BirthPlace']) && !preg_match("#^ +$#",$_POST['BirthPlace']) ) {
                $infos += ["birthplace"=>$_POST['BirthPlace']];
            }
            else {
                $infos += ["birthplace"=>null];
            }

            // Vérifie la nationalité sinon valeur=null
            if ( isset($_POST['Country']) && !preg_match("#^ +$#",$_POST['Country']) ) {
                $infos += ["country"=>$_POST['Country']];
            }
            else {
                $infos += ["country"=>null];
            }

            // Vérifie la motivation sinon valeur=null
            if ( isset($_POST['Motivation']) && !preg_match("#^ +$#",$_POST['Motivation']) ) {
                $infos += ["motivation"=>$_POST['Motivation']];
            }
            else {
                $infos += ["motivation"=>null];
            }
         ...
         * @return [tableau] retourne un tableau si les conditions sont respectés, et false sinon
         */
        public function check_data_x2(){
            // Vérifie le nom
            if ( isset($_POST['Id']) && !preg_match("#^ +$#",$_POST['Id']) ) {
                $infos = ["id"=>$_POST['Id']];
            }
            else {
                return false;
            ...
            // Vérifie le category
            if ( isset($_POST['Category']) && !preg_match("#^ +$#",$_POST['Category']) ) {
            ...
            // Vérifie le year
            if ( isset($_POST['Year']) && !preg_match("#^ +$#",$_POST['Year']) ) {
